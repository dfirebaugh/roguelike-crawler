...
class Controller extends Component {
  ...
  componentWillMount() {
    ...
    this.placer(this.gameObjs.enemies, arr, SIZE);
    this.placer(this.gameObjs.portal, arr, SIZE);
    this.placer(this.gameObjs.healthPack, arr, SIZE);
    this.placer(this.gameObjs.weapon, arr, SIZE);
    this.placer(this.gameObjs.dungeonMap, arr, SIZE);
  ...
  newLevel = () => {
    ...
    if (this.curFloor < 4) {
      ...
      this.placer(this.gameObjs.boss, arr, SIZE);
      // Boss(arr);
    }
  ...
  eachCell = (grid, fn) =>
    grid.forEach(row => row.forEach(cell => fn(cell, grid)));
  ...
  attack = (nextPos, grid) => {
    ...
    if (attackRoll >= 9) {
      ...
      this.setState({
        playerHealth: this.state.playerHealth - nextCell.attack
      });
      if (attackRoll > 4) {
  ...
  movePlayer = (dir, grid) => {
    ...
    //collision detection
    if (
      this.isWithinGrid(dir.nextPos, SIZE) &&
      this.getType(dir.nextPos, newGrid) !== "wall"
    ) {
      if (
        this.getType(dir.nextPos, newGrid) === "enemy" ||
        ...
        if (this.getType(dir.nextPos, newGrid) === "health") {
          this.combatLog(
            ...
            newGrid[dir.nextPos.y][dir.nextPos.x].addHealth +
            "hp!"
          );
        ...
        if (this.getType(dir.nextPos, newGrid) === "weapon") {
          this.combatLog(
            "Picked up " + newGrid[dir.nextPos.y][dir.nextPos.x].name + "!"
          );
          this.weapon = newGrid[dir.nextPos.y][dir.nextPos.x];
          this.weapon.attackPower =
            newGrid[dir.nextPos.y][dir.nextPos.x].attackPower;
        }
  ...
  isNear = pos => {
    let num = 5;
    return (
      pos.x - this.state.playerPos.x < num &&
  ...
  render() {
    let useCanvas = this.state.useCanvas ? (
      <CanvasGrid size={SIZE} level={this.curGrid} />
    ) : (
        <Grid
      ...
      <div>
        ...
        <p>
          {this.state.useCanvas
            ? "Currently rendering with Canvas"
            : "Currently rendering with DOM"}
        </p>
        <button onClick={this.handleRenderToggle}>
          {this.state.useCanvas
            ? "Switch to DOM Rendering"
            : "Switch to Canvas"}
        </button>
      </div>
    );
      ...
      <div>
        <div className="App-header">
          <StatusBox
            ...
            attackPower={
              this.weapon.attackPower +
              parseFloat((this.playerLevel * 5).toFixed(2))
            }
